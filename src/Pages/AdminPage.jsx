...
import { FaPlus, FaTrash, FaSignOutAlt, FaClipboardList, FaStore, FaCamera, FaEdit, FaTimes, FaCheckCircle, FaCalendarAlt } from "react-icons/fa";
...
const AdminPage = () => {
  ...
  const handleAddItem = (e) => {
    ...
    if (isEditing) {
      deleteItem(newItem.category, editId);
    }
    ...
    addItem(newItem.category, itemData);
    resetForm();
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
          <div>
            ...
            <p className="text-gray-400 font-bold text-xs uppercase tracking-widest mt-1">Management Dashboard</p>
          </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border  border-gray-100 sticky top-28 ">
                  ...
                  <form onSubmit={handleAddItem} className="space-y-5">
                    ...
                    <div>
                      ...
                      <select value={newItem.category} onChange={(e) => setNewItem({...newItem, category: e.target.value})} className="w-full bg-gray-50 border-none rounded-2xl px-4 py-4 font-bold">
                    ...
                    <div>
                      ...
                      <input type="text" value={newItem.name} onChange={(e) => setNewItem({...newItem, name: e.target.value})} className="w-full bg-gray-50 border-none rounded-2xl px-4 py-4 font-bold" />
                    </div>

                    <div>
                      <label className=" text-[10px] font-black uppercase text-gray-400  block">Price (৳)</label>
                      <input type="number" value={newItem.price} onChange={(e) => setNewItem({...newItem, price: e.target.value})} className="w-full bg-gray-50 border-none rounded-2xl px-4 py-4 font-bold" />
                    </div>

                    <button className={`w-full py-5 rounded-2xl font-black uppercase shadow-xl transition-all ${isEditing ? 'bg-blue-600 text-white' : 'bg-orange-600 text-white'}`}>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                  <table className="w-full text-left">
                    <thead className="bg-gray-50/50 border-b border-gray-100">
                      <tr>
                        <th className="px-8 py-5 font-black text-gray-400 uppercase text-[10px]">Preview</th>
                        <th className="px-8 py-5 font-black text-gray-400 uppercase text-[10px]">Product</th>
                        <th className="px-8 py-5 font-black text-gray-400 uppercase text-[10px]">Price</th>
                        <th className="px-8 py-5 font-black text-gray-400 uppercase text-[10px] text-right">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.entries(menu).map(([category, items]) => (
                        items.map((item, idx) => (
                          <tr key={`${category}-${idx}`} className="border-b border-gray-50 hover:bg-orange-50/20">
                            <td className="px-8 py-4"><img src={item.image} className="w-12 h-12 rounded-xl object-cover" /></td>
                            <td className="px-8 py-4">
                              <p className="font-bold text-gray-800">{item.name}</p>
                              <span className="text-[9px] font-black uppercase text-orange-500 bg-orange-50 px-2 py-0.5 rounded-md">{category}</span>
                            </td>
                            <td className="px-8 py-4 font-bold">৳{item.price}</td>
                            <td className="px-8 py-4 text-right space-x-2">
                              <button onClick={() => startEdit(category, item)} className="p-3 text-blue-400 hover:bg-blue-50 rounded-xl"><FaEdit /></button>
                              <button onClick={() => deleteItem(category, item.name)} className="p-3 text-red-300 hover:bg-red-50 rounded-xl"><FaTrash /></button>
                            </td>
                          </tr>
                        ))
                      ))}
          ...
          {activeTab === "orders" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="lg:col-span-3 space-y-6">
              <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
                <div className="flex items-center gap-4">
                    <div className="bg-orange-100 p-3 rounded-2xl text-orange-600"><FaClipboardList /></div>
                    <div>
                        <h2 className="text-xl font-black text-gray-800 uppercase tracking-tighter">Kitchen Queue</h2>
                        <p className="text-gray-400 text-xs font-bold uppercase">{orders.length} Active Orders</p>
                    </div>
                </div>
                {orders.length > 0 && (
                    <button onClick={clearAllOrders} className="text-red-500 text-xs font-black uppercase hover:underline">Clear History</button>
                )}
              </div>
              ...
              {orders.length > 0 ? (
                    ...
                    <motion.div layout key={order.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 relative overflow-hidden group">
                      ...
                      <div className="flex justify-between items-start mb-6">
                        <div>
                          ...
                          <p className="text-gray-400 text-[10px] font-bold mt-2 uppercase">{new Date(order.id).toLocaleTimeString()}</p>
                        </div>
                        <button 
                          onClick={() => completeOrder(order.id)} 
                          className="bg-green-500 text-white p-4 rounded-2xl hover:bg-green-600 shadow-lg shadow-green-100 transition-all"
                        >
                      ...
                      <div className="mt-6 pt-4 border-t-2 border-dashed border-gray-100 flex justify-between items-center">
                        <span className="text-gray-400 font-black uppercase text-xs">Total Amount</span>
                ...
                <div className="flex flex-col items-center justify-center py-40 bg-white rounded-[3rem] border-2 border-dashed border-gray-100">
                  <FaClipboardList size={50} className="text-gray-200 mb-4" />
                  <p className="text-gray-400 font-black uppercase text-sm tracking-widest">No pending orders</p>
                </div>
              )}
            </motion.div>
          )}
